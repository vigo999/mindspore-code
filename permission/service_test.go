...
func TestModePlan_DeniesWriteAndShell(t *testing.T) {
	svc := NewDefaultPermissionService(configs.PermissionsConfig{
		DefaultLevel: "ask",
		DefaultMode:  "plan",
	})

	if got := svc.Check("write", ""); got != PermissionDeny {
		t.Fatalf("Check(write) = %v, want %v", got, PermissionDeny)
	}
	if got := svc.Check("shell", "go test ./..."); got != PermissionDeny {
		t.Fatalf("Check(shell) = %v, want %v", got, PermissionDeny)
	}
}

func TestModeAcceptEdits_AllowsEditAndWrite(t *testing.T) {
	svc := NewDefaultPermissionService(configs.PermissionsConfig{
		DefaultLevel: "ask",
		DefaultMode:  "acceptEdits",
	})

	if got := svc.Check("edit", ""); got != PermissionAllowAlways {
		t.Fatalf("Check(edit) = %v, want %v", got, PermissionAllowAlways)
	}
	if got := svc.Check("write", ""); got != PermissionAllowAlways {
		t.Fatalf("Check(write) = %v, want %v", got, PermissionAllowAlways)
	}
}
